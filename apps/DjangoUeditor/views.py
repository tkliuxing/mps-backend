...
def get_ueditor_settings(request):
    json_data = json.dumps(
        ...
        ensure_ascii=False)
    call_back = request.GET.get("callback", None)
    if call_back:
        json_data = f'{call_back}({json_data});'
...
def get_files(root_path, cur_path, allow_types=[]):
    ...
    MEDIA_URL = getattr(USettings.gSettings, 'LOCAL_MEDIA_URL', USettings.gSettings.MEDIA_URL)
    for item in items:
...
def UploadFile(request):
    ...
    max_size = int(request.GET.get(upload_max_size[
    ...
    return_info = {
        # 保存后的文件名称
        'url': urljoin(MEDIA_URL, OutputPathFormat),
    ...
    json_data = json.dumps(return_info, ensure_ascii=False)
    call_back = request.GET.get("callback", None)
    if call_back:
        json_data = f'{call_back}({json_data});'
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext in allow_type:
            ...
            catcher_infos.append({
                "state": state,
                "url": urljoin(MEDIA_URL, o_path_format),
